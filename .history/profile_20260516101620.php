...
<body class="bg-light">
  ...
  ?>
    ...
    profile_form.addEventListener('submit', function(e) {
        ...
        xhr.onload = function() {
            // Ép kiểu thành chuỗi và xử lý
            let res = String(this.responseText).trim();

            if (res.includes('inv_img')) {
                alert('error', "Chỉ hỗ trợ định dạng JPG, WEBP & PNG!");
            } else if (res.includes('inv_size')) {
                alert('error', "Ảnh tải lên không được vượt quá 2MB!");
            } else if (res.includes('upd_failed')) {
                alert('error', "Tải hình ảnh thất bại!");
            } else if (res === '0') {
                alert('error', "Cập nhật thất bại!");
            } else if (res.includes('1')) {
                // CHỈ CẦN CÓ SỐ 1 LÀ THÀNH CÔNG
                alert('success', 'Cập nhật ảnh đại diện thành công!');
                setTimeout(() => {
                    window.location.href = window.location.pathname;
                }, 1000);
            } else {
                alert('error', "Lỗi từ máy chủ: " + res);
            }
        }
