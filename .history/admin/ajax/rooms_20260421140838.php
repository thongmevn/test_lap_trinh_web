<?php 
  ...
  if(isset($_POST['get_all_rooms']))
  {
    ...
    while($row = mysqli_fetch_assoc($res))
    {
      if($row['status']==1){
        ...
        style='background:#212529;color:#fff;border:none;padding:4px 8px;border-radius:4px;cursor:pointer;'>
        active</button>";
      }
      else{
        $status = "<button onclick='toggle_status($row[id],1)' 
        style='background:#ffc107;color:#000;border:none;padding:4px 8px;border-radius:4px;cursor:pointer;'>
        inactive</button>";
      }
      ...
      $data.="
        <tr style='vertical-align:middle;'>
          ...
          <td>$row[area] m2</td>
          <td>
            <span style='display:inline-block;background:#f8f9fa;color:#212529;padding:5px 10px;border-radius:50px;margin-bottom:5px;'>
               Adult: $row[adult]
            </span><br>

            <span style='display:inline-block;background:#f8f9fa;color:#212529;padding:5px 10px;border-radius:50px;'>
          ...
          <td>
            ...
            style='background:#0d6efd;color:#fff;border:none;padding:5px 8px;border-radius:4px;cursor:pointer;'>
              <i class='bi bi-pencil-square'></i>
            </button>

            <button type='button' onclick=\"room_images($row[id],'$row[name]')\"
            data-bs-toggle='modal' data-bs-target='#room-images'
            style='background:#0dcaf0;color:#000;border:none;padding:5px 8px;border-radius:4px;cursor:pointer;'>
              <i class='bi bi-images'></i>
            </button>

            <button type='button' onclick='remove_room($row[id])'
            style='background:#dc3545;color:#fff;border:none;padding:5px 8px;border-radius:4px;cursor:pointer;'>
  ...
  if(isset($_POST['edit_room']))
  {
    ...
    if($stmt = mysqli_prepare($con,$q2))
    {
      foreach($facilities as $f){
        ...
        if(!mysqli_stmt_execute($stmt)){ // ✅ Kiểm tra từng execute
          $flag = 0;                     // Nếu 1 cái lỗi → flag = 0
  ...
  if(isset($_POST['get_room_images']))
  {
    ...
    {
      if($row['thumb']==1){
        $thumb_btn = "<span style='background:#198754;color:#fff;padding:5px 8px;border-radius:4px;font-size:16px;'>
          <i class='bi bi-check-lg'></i>
        </span>";
      }
      else{
        $thumb_btn = "<button onclick='thumb_image($row[sr_no],$row[room_id])'
          style='background:#6c757d;color:#fff;border:none;padding:5px 8px;border-radius:4px;cursor:pointer;'>
          <i class='bi bi-check-lg'></i>
        </button>";
      }
      ...
      echo<<<data
        <tr style="vertical-align:middle;">
          <td>
            <img src='$path$row[image]' style='width:100%;height:auto;'>
          </td>
          <td>$thumb_btn</td>
          <td>
