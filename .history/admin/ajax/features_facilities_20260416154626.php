    ...
    while($row = mysqli_fetch_assoc($res))
    {
      echo <<<data
        <tr>
          ...
          <td>$row[name]</td>
          <td>
            <button type="button" onclick="rem_feature($row[id])" class="btn-delete">
  ...
  if(isset($_POST['get_facilities']))
  {
    ...
    while($row = mysqli_fetch_assoc($res))
    {
      echo <<<data
        <tr class='row-middle'>
          <td>$i</td>
          <td><img src="$path$row[icon]" class="facility-img"></td>
          <td>$row[name]</td>
          <td>$row[description]</td>
          <td>
            <button type="button" onclick="rem_facility($row[id])" class="btn-delete">
              <i class="bi bi-trash"></i> Xoá
            </button>
          </td>
        </tr>
