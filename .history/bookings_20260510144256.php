...
<body class="bg-light">
        ...
        while($data = mysqli_fetch_assoc($result))
        {
          ...
          if($data['booking_status']=='booked')
          {
            ...
            else
            {
              ...
              if($data['payment_status']=='unpaid'){
                $btn="
                <button onclick='pay_deposit($data[booking_id])' 
                class='btn btn-success btn-sm shadow-none'>Thanh toán cọc</button>

                <button onclick='cancel_booking($data[booking_id])' 
                class='btn btn-danger btn-sm shadow-none ms-2'>Huỷ & hoàn tiền</button>
                ";
              }

              // 🔴 ĐÃ THANH TOÁN CỌC
              else if($data['payment_status']=='deposited'){
                $btn="
          ...
          else if($data['booking_status']=='cancelled')
          {
            ...
            if($data['payment_status']=='refunded'){
              $btn="<span class='badge bg-success'>Đã hoàn tiền</span>";
            }
            else if($data['payment_status']=='no_refund'){
              $btn="<span class='badge bg-danger'>Không hoàn tiền</span>";
            }
            else{
              $btn="<span class='badge bg-warning'>Đang xử lý</span>";
            }
          }
          else
          {
            ...
            <div class='col-md-4 px-4 mb-4'>
              <div class='bg-white p-3 rounded shadow-sm'>
                ...
                <p>
                  ...
                  <b>Deposit: </b> $data[deposit] VND <br>
  ...
  ?>
    ...
    function pay_deposit(id) {
        if (confirm('Bạn có muốn thanh toán tiền cọc không?')) {
            let xhr = new XMLHttpRequest();
            ...
            xhr.onload = function() {
                ...
                let res = this.responseText.trim(); // 🔥 QUAN TRỌNG

                if (res == "1") {
                    alert('Thanh toán thành công!');
                    location.reload();
                } else {
                    alert('Có lỗi xảy ra: ' + res);
                    console.log(res);
                }
            }

            xhr.send('pay_deposit=1&id=' + id);
        }
    }
    </script>
