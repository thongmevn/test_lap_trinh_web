...
<head>
    ...
    #roomCarousel:hover .carousel-control-prev,
    #roomCarousel:hover .carousel-control-next {
        opacity: 1;
  ...
  ?>
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-7 col-md-12 px-4">
                <!-- Main Carousel -->
                <div id="roomCarousel" class="carousel slide rounded-3 overflow-hidden shadow-sm mb-3"
            ...
            ?>
                        ...
                        <div
                            style="position: absolute; bottom: 15px; right: 15px; background: rgba(0,0,0,0.7); color: white; padding: 8px 15px; border-radius: 20px; font-size: 14px; z-index: 10;">
                            <span id="currentImageNum">1</span> / <span
                                id="totalImageNum"><?php echo $image_count; ?></span>
                        </div>
                        <?php endif; ?>
                    </div>
            ...
            while($img_res = mysqli_fetch_assoc($img_q)) {
              ...
              echo "
                <img src='$thumb_img' class='rounded cursor-pointer thumb-img $thumb_active' style='min-width: 80px; width: 80px; height: 80px; object-fit: cover; cursor: pointer; transition: all 0.3s;' data-thumb-index='$thumb_index'>
              ";
              $thumb_index++;
            }
          ?>
    ...
    if (carouselElement) {
        carouselElement.addEventListener('slid.bs.carousel', function(e) {
            const totalImages = document.getElementById('totalImageNum');
            if (totalImages) {
                const currentIndex = e.to + 1;
                document.getElementById('currentImageNum').textContent = currentIndex;
            }
    ...
    function updateThumbnailActive() {
        ...
        const activeIndex = Array.from(carouselElement.querySelectorAll('.carousel-item')).indexOf(activeItem);
        const thumbnails = document.querySelectorAll('#thumbnailCarousel img');

        thumbnails.forEach((thumb, index) => {
            if (index === activeIndex) {
                thumb.classList.remove('border-light', 'border-2');
                thumb.classList.add('border-primary', 'border-3');
            } else {
                thumb.classList.remove('border-primary', 'border-3');
                thumb.classList.add('border-light', 'border-2');
            }
        });
    }

    // Thumbnail click handler
    const thumbnails = document.querySelectorAll('#thumbnailCarousel img.thumb-img');
    thumbnails.forEach(thumb => {
        thumb.addEventListener('click', function() {
            const index = parseInt(this.getAttribute('data-thumb-index'));
            carousel.to(index);
            updateThumbnailActive();
        });
